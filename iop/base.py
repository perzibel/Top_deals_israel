...
class IopRequest(object):
    def __init__(self,api_pame,http_method = 'GET'):
        self._api_params = {}
        self._file_params = {}
...
class IopClient(object):
    ...
    def execute(self, request,access_token = None):
        ...
        try:
            if(request._http_method == 'GET' or len(request._file_params) != 0) :
                r = requests.get(api_url,sign_parameter,files=request._file_params, timeout=self._timeout)
            else:
        ...
        try:
            ...
            raise Exception(
                f"AliExpress API returned non-JSON response. "
                f"Status={r.status_code}, Text={r.text[:1000]}"
            ) from e
